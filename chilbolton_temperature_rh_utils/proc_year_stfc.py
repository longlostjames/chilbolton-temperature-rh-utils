...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Process a full year of HMP155 data from STFC variant.",
        epilog="""This command processes an entire year of Vaisala HMP155 temperature and 
relative humidity data from Campbell Scientific CR1000X datalogger format (STFC 
variant) to CF-compliant NetCDF files with automated QC flagging."""
    )
    ...
    while current_date <= end_date:
        ...
        ncfile = outdir / f"stfc-temperature-rh-1_cao_{date_str}_surface-met_v1.1.nc"
        
        # Add QC flags for purge times
        if ncfile.exists():
            try:
                # Build arguments for flag_purge_main
                flag_args = [str(ncfile)]
